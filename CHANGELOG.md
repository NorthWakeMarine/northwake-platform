...
- **One-time cleanup**: `POST /api/fix-needs-attention` (auth-gated) re-derives a real stage for contacts already stranded in `needs_attention` from their QuickBooks timeline (paid/invoiced → `done_invoiced`, estimate → `estimate_sent`, else `new_leads`), since the original stage was unrecoverable.
- **Phone matching hardened** (related, separate defect): known callers whose stored phone wasn't E.164 were missed by `findContactByPhone`, so Quo created duplicate "NEW LEAD + number" cards. Matching now falls back to a digit-suffix lookup and re-normalizes candidates, and the board dedup (`getPipelineBoard`) normalizes phones on both sides. New shared helper `src/lib/phone.ts`. One-time `POST /api/backfill-phones` canonicalizes stored phones to E.164.
