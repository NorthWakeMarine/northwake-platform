  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {services.map((s) => {
        ...
        if (isEditingThis) {
          return (
            <form key={s.id} action={editAction} className="border border-slate-200 rounded-sm p-3 flex flex-col gap-2">
              <input type="hidden" name="service_id"  value={s.id} />
              <input type="hidden" name="contact_id"  value={contactId} />
              <div className="flex flex-col gap-1">
                <label className="text-[10px] tracking-widest uppercase font-medium text-slate-400">Service</label>
                <input name="service_name" list="edit-service-suggestions" required defaultValue={s.service_name}
              ...
              <div className="flex flex-col gap-1">
                ...
                <select name="interval_days" defaultValue={s.interval_days}
          ...
          <div key={s.id} className="border border-slate-100 rounded-sm p-3 flex flex-col gap-2">
            <div className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2.5">
                <span className={`text-[9px] ${notifOn ? "text-[#000080]" : "text-slate-300"}`} title={notifOn ? "Notifications on" : "Notifications off"}>
...
function NotificationsField({ defaultValue }: { defaultValue: boolean }) {
